...
class NewGradedCriteria extends Component {
  constructor(props) {
    super(props);
    this.state = {
      question: {
      ...
      .catch(error => {
        if (error.status === 401) {
          this.props.handleLogout(
            "/login",
            "error",
            "You have been logged out. Please login create criteria."
          );
        } else {
          notification.error({
            message: "Smart Team",
...
export default NewGradedCriteria;
